...
int password_change(void)
{
	...
	//Running loop to obtain the password entry
	while(1)
	{
		...
		//Read the switch press (SW1 = 1; SW2 = 2)
		if (count < 4  && (key == MK_SW1 || key == MK_SW2))
		{
			...
			//Prompt to re-enter the new password again
			if (count == 4)
			{
				CLEAR_DISP_SCREEN;
				pos = 0;
		...
		//Below section receives the re-entered password
		...
		//Comparison between the entered password and re-entered password and display output based on result
		...
		else if (count == 9  && (strcmp(c_password, r_password) != 0))
		{
			...
			//Calling the delay function before the Menu screen re-appears
			delay_fun();
			return 1;
...
//Defining the View Log function
void view_log(void)
{
	//Declare the variables
	...
	while (1)
	{
		...
		//Switch key 7 returns the user fron the View log section to the Menu screen
		if (key1 == MK_SW7)
		{
			if (count++ >= 1000)
			{
		...
		//Switches 11 and 12 are used to scroll UP and DOWN while in the View Log section
		if (key == MK_SW11 || key == MK_SW12)
		{
			...
			//Below section is to read the data from the EEPROM
